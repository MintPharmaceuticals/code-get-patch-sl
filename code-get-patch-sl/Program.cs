...
namespace code_get_patch_sl
{
    ...
    {
        private static string baseUrl = "http://vmsapdevcc01.mintpharmaceuticals.loca:50001/b1s/v1"; // Replace with your SAP Service Layer base URL
        ...
        static void Main(string[] args)
        {
            ...
            {
                GetInvoice(sessionId, "133");
                //PatchBpContact(sessionId, "C20000");
                //GetBatchNumber(sessionId, "5702");
                Logout(sessionId);
            }
            else
            {
        ...
        private static void GetBatchNumber (string sessionId, string docEntry) {
            string getUrl = $"{baseUrl}/BatchNumberDetails({docEntry})";
            try {
                ...
                using (HttpWebResponse response=(HttpWebResponse)request.GetResponse()) {
                    ...
                    if (response.StatusCode == HttpStatusCode.OK) {
                        ...
                        var readItemDesc = (string)jsonObject["ItemDescription"];
                        var readItemCode = (string)jsonObject["ItemCode"];

                        Console.WriteLine (string.Format("Batch retrieved successfully with DocEntry ={0} and ItemDescription = {1} for ItemCode = {2}", readDocEntry, readItemDesc, readItemCode));
                    } else {
                        Console.WriteLine("Failed to retrieve Batch Number. Status code: " + response.StatusCode);
                    }
                }
            } catch (Exception ex){
                Console.WriteLine("An error occured while retrieving: " + ex.Message);
            }
        }
